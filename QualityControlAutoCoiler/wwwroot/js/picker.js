...
(function (factory) {
    ...
    function PickerConstructor(ELEMENT, NAME, COMPONENT, OPTIONS) {
        ...
        var IS_DEFAULT_THEME = !1, STATE = { id: ELEMENT.id || 'P' + Math.abs(~~(Math.random() * new Date())) }, SETTINGS = COMPONENT ? $.extend(!0, {}, COMPONENT.defaults, OPTIONS) : OPTIONS || {}, CLASSES = $.extend({}, PickerConstructor.klasses(), SETTINGS.klass), $ELEMENT = $(ELEMENT), PickerInstance = function () { return this.start() }, P = PickerInstance.prototype = {
            constructor: PickerInstance, $node: $ELEMENT, start: function () {
                ...
                if (dontGiveFocus !== !1) {
                    ...
                    $document.on('click.' + STATE.id + ' focusin.' + STATE.id, function (event) {
                        ...
                        if (keycode == 27) { P.close(!0) } else if (target == P.$holder[0] && (keycodeToMove || keycode == 13)) {
                            event.preventDefault()
                            if (keycodeToMove) { PickerConstructor._.trigger(P.component.key.go, P, [PickerConstructor._.trigger(keycodeToMove)]) } else if (!P.$root.find('.' + CLASSES.highlighted).hasClass(CLASSES.disabled)) {
                ...
                if (giveFocus) {
                    if (SETTINGS.editable) { ELEMENT.focus() } else {
        ...
        function prepareElementHolder() {
            P.$holder.on({
                ...
                if (!targetDisabled && targetData.nav) { P.set('highlight', P.component.item.highlight, { nav: targetData.nav }) } else if (!targetDisabled && 'pick' in targetData) {
